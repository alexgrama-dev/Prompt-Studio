...
- [ ] 1.1 Record the `q + 15` benchmark, initialize Browse from first-fallback text, paste only the selected visible result, and cover no-result safety in `test/core.test.mts`
- [ ] 1.2 Add the live filled-body preview with repeated and blank placeholder coverage in `test/core.test.mts`
...
- [ ] 2.1 Prove the same first-paste query falls from `q + 15` to `q + 3` and verify all changed Raycast panels on the MacBook
