...
- [x] 1.1 Record the `q + 15` benchmark, initialize Browse from first-fallback text, paste only one unique exact full-identifier, title, or alias match without placeholders, and cover ambiguous, partial, semantic, archived, placeholder, and no-result safety in `test/launch-context.test.mts`
  - 2026-07-29: Prompt Studio became the first MacBook fallback. The exact saved title pasted in `q + 1` keys after the query, which beats the `q + 2` maximum. Ambiguous, partial, semantic, archived, placeholder-bearing, and no-result regressions passed with `pnpm test` at 85/85; `pnpm typecheck` and `pnpm lint` exited 0.
...
- [ ] 2.1 Prove the same exact first-paste query falls from `q + 15` to no more than `q + 2` and verify all changed Raycast panels on the MacBook
- [x] 2.2 Pass `pnpm check` on the Mac Mini mirror
- [x] 2.3 Pass `pnpm check:store`
- [x] 2.4 Pass `openspec validate --all --strict`
- [x] 2.5 Record dated evidence in `docs/verification/2026-07-29-fast-daily-reuse.md`
