...
- [x] 7.23 Keep the saved-prompt preview copy-ready by removing library tags and other supporting metadata from beneath the prompt body
  - 2026-07-21: the Raycast preview already ended with Full Prompt after 7.21/7.22, so the remaining offenders were the CLI `get` human output and the MCP `get` text, which printed ID, Target, and Tags beneath the body. Both now print that metadata above the body so every saved-prompt rendering ends with the copy-ready prompt. The Mac Mini mirror verification is recorded with this change's commit.
