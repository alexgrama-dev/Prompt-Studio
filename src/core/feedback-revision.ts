...
export function buildFeedbackRevisionThoughts(
  ...
  if (feedback.length === 0) {
    throw new Error(
      "A feedback revision needs at least one recorded feedback entry.",
    );
  }
  const sections = feedback.map((record, index) => {
    ...
    if (record.finalPrompt) {
      lines.push(
        `- Prompt text the user actually ran:\n\n${indent(clip(record.finalPrompt))}`,
      );
    }
...
function clip(value: string): string {
  ...
  return trimmed.length > MAX_FIELD_CHARACTERS
    ? `${trimmed.slice(0, MAX_FIELD_CHARACTERS)}…`
    : trimmed;
}
